...
async function renderCorpoRelatorio(area, state, auditoriaId) {
  ...
  const agendaPorDia = {};
  agenda.forEach((b) => { (agendaPorDia[b.dia] = agendaPorDia[b.dia] || []).push(b); });
  ...
  area.innerHTML = `
    ...
    <div class="planejamento-box">
      ...
      ${Object.keys(agendaPorDia).length ? Object.entries(agendaPorDia).map(([dia, blocos]) => `
        <p style="font-weight:600;margin:8px 0 4px">Dia ${dia}</p>
        <table class="table">
          <thead><tr><th style="width:140px">Horário</th><th>Atividade</th><th>Auditor(es)</th></tr></thead>
          <tbody>${blocos.map((b) => `
            <tr>
              <td>${b.hora_inicio.slice(0, 5)} às ${b.hora_fim.slice(0, 5)}</td>
              <td>${escapeHtml(b.rotulo || '')}</td>
              <td>${b.auditor_sugerido_id ? escapeHtml(nomeAuditorRel(b.auditor_sugerido_id)) : '—'}${b.auditor_sugerido_2_id ? ' e ' + escapeHtml(nomeAuditorRel(b.auditor_sugerido_2_id)) : ''}</td>
            </tr>`).join('')}</tbody>
        </table>
      `).join('') : '<p class="text-muted" style="font-size:12px">Nenhuma agenda gerada ainda para esta auditoria — gere em "Auditorias" → planejamento inteligente.</p>'}
    </div>
...
function imprimirRelatorioDetalhado(auditoria, itens, pessoas, instrumentos, procedimentos, conclusaoTexto, nomeProcesso, agendaPorDia, nomeAuditor, equipe, achados) {
  ...
  imprimirSecao(`
    ...
    ${agendaPorDia && Object.keys(agendaPorDia).length ? Object.entries(agendaPorDia).map(([dia, blocos]) => `
      <p style="font-weight:600;margin:8px 0 4px">Dia ${dia}</p>
      <table class="table">
        <thead><tr><th>Horário</th><th>Atividade</th><th>Auditor(es)</th></tr></thead>
        <tbody>${blocos.map((b) => `
          <tr>
            <td>${b.hora_inicio.slice(0, 5)} às ${b.hora_fim.slice(0, 5)}</td>
            <td>${escapeHtml(b.rotulo || '')}</td>
...
async function renderDashboard(container, state) {
  ...
  area.innerHTML = `
    <div class="stats-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;margin-bottom:1.25rem">
      ...
      <div class="stat-box accent-pink"><div class="text-muted" style="font-size:12px">Ações em atraso</div><div style="font-size:22px;font-weight:700;color:${acoesEmAtraso ? '#ef4444' : 'inherit'}">${acoesEmAtraso}</div></div>
      <div class="stat-box accent-green"><div class="text-muted" style="font-size:12px">Ações concluídas</div><div style="font-size:22px;font-weight:700">${acoesConcluidas}</div></div>
      <div class="stat-box accent-slate"><div class="text-muted" style="font-size:12px">Horas planejadas (total)</div><div style="font-size:22px;font-weight:700">${horasPlanejadas.toFixed(1)}h</div></div>
    </div>
