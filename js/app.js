...
import { aplicarTema, extrairCoresDoLogo } from './tema.js';
...
async function carregarEmpresas() {
  ...
  state.empresas = (data || [])
    .filter((v) => v.empresas)
    .map((v) => ({ ...v.empresas, papel: v.papel }));
...
function abrirModalNovaEmpresa() {
  ...
  modal.querySelector('#form-nova-empresa').addEventListener('submit', async (e) => {
    ...
    const { error: errUpd } = await supabase.from('empresas')
      .update({ cor_primaria: corPrimaria, cor_destaque: corDestaque, logo_url: logoUrl })
