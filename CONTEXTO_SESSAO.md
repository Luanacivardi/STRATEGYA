...
### Pendente — itens grandes, para as próximas fases (uma de cada vez)
- **Fase 13 — Versionamento anual do Planejamento Estratégico:** precisa de decisão de modelagem — either (a) "fechar o ano" congela uma cópia/snapshot de objetivos/indicadores/mapa/planos daquele ano e abre um novo ciclo editável, ou (b) todo registro ganha `ano_referencia` e a tela vira um filtro por ano sobre os dados vivos. Definir com Luana antes de mexer no schema.
- **Fase 14 — Rastreabilidade completa (auditoria):** tabela `log_alteracoes` (empresa_id, tabela, registro_id, usuário, timestamp, campo, valor_anterior, valor_novo) + triggers `AFTER UPDATE/INSERT/DELETE` nas tabelas principais (objetivos, indicadores, planos, atas, riscos, usuários_empresas...). Tela de consulta do histórico (provavelmente dentro de Configurações ou Permissões, só ORBEEX/admin). Maior item do backlog — schema afeta o banco inteiro.
- **Fase 15 — E-mail:** revisar `enviarPorEmail()` (hoje só abre `mailto:` do cliente local, sem registro de envio nem anexo); avaliar Edge Function com um provedor de e-mail transacional (Resend/SendGrid) para: registrar envios (tabela `emails_enviados`), permitir anexos nas ações, e gerar sempre um PDF do conteúdo antes de enviar (reaproveitando `imprimirSecao`/impressão existente, convertendo para PDF no servidor).
- **Fase 16 — Segurança e departamentos:** validação por e-mail no primeiro acesso (Supabase Auth já suporta confirmação de e-mail — avaliar ativar); nova tabela `departamentos` por empresa; `usuarios_empresas` ganha `departamento_id` opcional; permissões de módulo/acesso poderão ser concedidas por usuário individual OU por departamento inteiro (precisa decidir precedência quando as duas coexistem).
- **Regras já confirmadas como implementadas** (não precisam de trabalho novo): usuários com papel "usuario" já não conseguem incluir/editar/excluir colaboradores nem acessar Configurações (`podeGerenciar` em `empresaUsuarios.js`, tela só visível a admin+orbeex).
